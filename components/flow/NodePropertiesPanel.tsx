...
import { HelpCircle, Trash2, ChevronRight, DollarSign } from "lucide-react";
      ...
      <SheetContent side="right" className="w-[480px] sm:w-[540px] p-0 flex flex-col h-screen bg-white dark:bg-gray-950">
        ...
        {selectedNode && (
          <Tabs defaultValue="configuration" className="flex-1 flex flex-col">
            {/* Show tabs only for action nodes with app data */}
                        ...
                        setNodes((ns) =>
                          ns.map((n) =>
                            n.id === selectedNode.id
                              ? { ...n, data: recalculateNodeWidth({ ...n.data, label: newLabel }) }
                              : n
                          )
                        );
                      }}
                              ...
                              <Select
                                ...
                                onValueChange={(value) => {
                                  setNodes((ns) =>
                                    ns.map((n) =>
                                      n.id === selectedNode.id
                                        ? { ...n, data: recalculateNodeWidth({ ...n.data, contextValue: value }) }
                                        : n
                                    )
                                  );
                                }}
                                      ...
                                      <Checkbox
                                        ...
                                        onCheckedChange={(checked) => {
                                          ...
                                          setNodes((ns) =>
                                            ns.map((n) =>
                                              n.id === selectedNode.id
                                                ? { ...n, data: recalculateNodeWidth({ ...n.data, contextValue }) }
                                                : n
                                            )
                                          );
                                        }}
                              ...
                              onChange={(e) => {
                                ...
                                if (!template.multiSelect) {
                                  setNodes((ns) =>
                                    ns.map((n) =>
                                      n.id === selectedNode.id
                                        ? { ...n, data: recalculateNodeWidth({ ...n.data, contextValue: customValue }) }
                                        : n
                                    )
                                  );
                                }
                  ...
                  {(() => {
                    ...
                    if (nodeData?.pricingData && nodeData?.appId) {
                      ...
                      if (selectedTier === 'free' || (selectedTier === 'default' && nodeData.pricingData.hasFreeTier)) {
                        ...
                        // Rough estimate: $0.01 per operation
                        appCostForNode = runsPerMonth * 0.01 * (selectedNode.type === 'action' ? 1.5 : 1);
                      } else if (selectedTier === 'custom') {
                        // For custom pricing, use a conservative estimate
                        appCostForNode = nodeData.pricingData.lowestMonthlyPrice || 100; // Default to $100 for custom
                      } else {
                        // Default: use lowest price if no free tier
                        appCostForNode = nodeData.pricingData.hasFreeTier ? 0 : (nodeData.pricingData.lowestMonthlyPrice || 0);
                      }
                      
                      // For nodes that are part of a workflow, divide the app cost by the number of nodes using the same app
              ...
              <TabsContent value="pricing" className="flex-1 overflow-y-auto mt-0">
                <div className="p-6 space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm bg-muted/30 p-4 rounded-lg">
                      ...
                      {nodeData?.pricingData?.isPricingPublic === false && (
                        <>
                          ...
                          <div className="font-medium">Custom/Contact Sales</div>
                        </>
                      )}
                    </div>
                    ...
                    {/* Tier Selector */}
                    <div>
                      <Label className="text-sm">Selected Pricing Tier</Label>
                      <Select
                        value={nodeData.selectedTier || "default"}
                        onValueChange={(value) => {
                          setNodes((ns) =>
                            ns.map((n) =>
                              n.id === selectedNode.id
                                ? { ...n, data: { ...n.data, selectedTier: value } }
                                : n
                            )
                          );
                        }}
                        ...
                        <SelectContent>
                          {nodeData?.pricingData?.hasFreeTier && (
                            <SelectItem value="free">
                              <div className="flex flex-col">
                                ...
                                <span className="text-xs text-muted-foreground">No monthly cost</span>
                              </div>
                            </SelectItem>
                          )}
                          
                          {nodeData?.pricingData?.lowestMonthlyPrice !== null && nodeData?.pricingData?.lowestMonthlyPrice !== undefined && nodeData?.pricingData?.lowestMonthlyPrice > 0 && (
                            <SelectItem value="starter">
                              <div className="flex flex-col">
                                <span className="font-medium">Starter</span>
                                <span className="text-xs text-muted-foreground">
                                  From {nodeData?.pricingData?.currency || '$'}{nodeData?.pricingData?.lowestMonthlyPrice}/mo
                                </span>
                              </div>
                            </SelectItem>
                          )}
                          ...
                          <SelectItem value="default">
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-muted-foreground">Use recommended tier</span>
                            </div>
                          </SelectItem>
                      ...
                      <p className="text-xs text-muted-foreground mt-2">
                        This affects ROI calculations for this specific app integration.
                      </p>
                    </div>
                    ...
                    <div className="space-y-3 pt-3 border-t">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-muted-foreground">Runs per month:</span>
                          <span className="font-medium tabular-nums">{runsPerMonth.toLocaleString()}</span>
                        </div>
                        
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-muted-foreground">Operations per run:</span>
                          <span className="font-medium tabular-nums">
                            {selectedNode.type === 'action' ? '1-2' : '1'}
                          </span>
                        </div>
                        
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-muted-foreground">Est. monthly operations:</span>
                          <span className="font-medium tabular-nums">
                            {(runsPerMonth * (selectedNode.type === 'action' ? 1.5 : 1)).toLocaleString()}
                          </span>
                        </div>
                        
                        {/* Cost Estimation based on selected tier */}
                        {(() => {
                          const selectedTier = nodeData.selectedTier || 'default';
                          let estimatedCost = 0;
                          ...
                          if (selectedTier === 'free') {
                            ...
                            estimatedCost = nodeData.pricingData.lowestMonthlyPrice;
                          } else if (selectedTier === 'usage' && nodeData.pricingData.hasUsageBasedPricing) {
                            // Rough estimate: $0.01 per operation
                            estimatedCost = runsPerMonth * 0.01 * (selectedNode.type === 'action' ? 1.5 : 1);
                          } else if (selectedTier === 'custom') {
                            estimatedCost = -1; // Indicates custom pricing
                          } else {
                            // Default: use lowest price if available
                            estimatedCost = nodeData.pricingData.lowestMonthlyPrice || 0;
                          }
                          
                          return (
                            <div className="mt-3 pt-3 border-t">
                              <div className="flex items-center justify-between text-sm">
                                <span className="text-muted-foreground">Est. app cost:</span>
                                <span className="font-medium">
                                  {estimatedCost === -1 
                                    ? 'Contact for pricing' 
                                    : `${nodeData.pricingData.currency || '$'}${estimatedCost.toFixed(2)}/mo`
                                  }
                                </span>
                              </div>
                              ...
                              {estimatedCost > 0 && (
                                <div className="mt-2 text-xs text-muted-foreground">
                                  This is an estimate. Actual costs may vary based on usage and features.
                                </div>
                              )}
                            </div>
                          );
                    ...
                    {/* Additional App Info */}
                    {nodeData.pricingData.priceModelType && nodeData.pricingData.priceModelType.length > 0 && (
                      <div className="space-y-2 pt-3 border-t">
                        <h4 className="text-sm font-medium">Pricing Models</h4>
                        <div className="flex flex-wrap gap-1">
                          {nodeData.pricingData.priceModelType.map((model, idx) => (
