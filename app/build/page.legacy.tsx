...
function BuildPageContent() {
  ...
  useEffect(() => {
    ...
    async function manageScenario() {
      ...
      if (templateIdParam && scenarioToLoad && activeScenarioIdToLoad && (!scenarioToLoad.nodesSnapshot || scenarioToLoad.nodesSnapshot.length === 0)) {
        try {
          ...
          primaryTemplateData = await res.json();
              ...
              // Cache template pricing data for future use
              if (templateIdParam) {
      ...
      // BUT only if we don't already have them cached or if the query changed
      if (queryParam && activeScenarioIdToLoad && scenarioToLoad) {
        const shouldFetchAlternatives = 
          !scenarioToLoad.alternativeTemplatesCache || 
          scenarioToLoad.alternativeTemplatesCache.length === 0 ||
          scenarioToLoad.searchQuery !== queryParam;
        ...
        } // Close shouldFetchAlternatives if
      } else if (queryParam && activeScenarioIdToLoad && scenarioToLoad && scenarioToLoad.alternativeTemplatesCache) {
        // We have cached alternatives, just restore them from the database
        const cachedAlternatives: Scenario[] = (scenarioToLoad.alternativeTemplatesCache as Array<{
          templateId?: string;
          title: string;
          platform: string;
          nodesCount: number;
          ...
          createdAt: Date.now(),
          updatedAt: Date.now(),
          platform: cache.platform as LibPlatformType,
          nodesSnapshot: [],
          edgesSnapshot: [],
          originalTemplateId: cache.templateId,
          searchQuery: scenarioToLoad.searchQuery || queryParam,
          runsPerMonth: 250,
          minutesPerRun: 3,
          hourlyRate: 30,
          taskMultiplier: 1.5,
        ...
        setAlternativeTemplates(cachedAlternatives);
      }
      setIsLoading(false);
    }
    manageScenario();
