...
export function getDefaultFactors(
  ...
  const defaultPositiveFactors: Record<TaskType, PositiveFactor[]> = {
    ...
    admin: [], // Will be filled with similar pattern
    customer_support: [],
    sales: [],
    marketing: [],
    compliance: [],
  ...
  // For now, return general factors for all types
  // In production, each task type would have specific factors
  const positiveFactors = defaultPositiveFactors[taskType] || defaultPositiveFactors.general;
  const negativeFactors = defaultNegativeFactors[taskType] || defaultNegativeFactors.general;
